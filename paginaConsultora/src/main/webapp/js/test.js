		...
		$('.open-popup-link').magnificPopup({
		  ...
		  callbacks: {
		    open: function() {
		      ...
		      this.content.find('.image img').css('max-height', ($(window).height() * 0.7) + 'px');
		      this.content.css('width', (this.content.find('.image img').width()) + 'px');
		    },
  		  }
		});
}
...
function loadsuperslides(){
	...
	$('#slides-1').bind('animating.slides', function(e, orientation) {
		var $current = $('.main-title').find('ul li:eq(' + orientation.outgoing_slide + ')');
		var $next    = $('.main-title').find('ul li:eq(' + orientation.upcoming_slide + ')');

		if($current.size() == 0 || $next.size() == 0)
			return;

		$current.removeClass('t-current');
		$next.addClass('t-current');
	});
}
